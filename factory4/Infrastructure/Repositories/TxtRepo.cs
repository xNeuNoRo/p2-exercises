...
// Ya que de esa forma podemos crear instancias de T dentro de la clase y asignar manualmente sus propiedades 1 por 1
{
    ...
    // SINGLETON
    private static TxtRepo<T>? _instance;
    ...
    public static TxtRepo<T> GetInstance(string filePath)
    {
        if (_instance == null)
        {
            _instance = new TxtRepo<T>(filePath);
        }
    ...
    public List<T> Load()
    {
        ...
        var blocks = content.Split([BlockSeparator], StringSplitOptions.RemoveEmptyEntries);
    ...
    private T ParseBlock(string block)
    {
        ...
        foreach (var line in lines)
        {
            ...
            prop?.SetValue(item, parts[1].Trim());
        }
