...
public static class String
{
    ...
    // Asegura que un path tenga la extension dada. Si no la tiene, la agrega.
    public static string EnsureExtension(string path, string extension)
    {
        // Si el path es nulo o vacio, retornarlo tal cual
        if (string.IsNullOrWhiteSpace(path))
            return path;

        // Normalizamos la extension para que siempre empiece con un punto
        string dottedExtension = extension.StartsWith('.') ? extension : "." + extension;

        // Si el path ya termina con la extension (ignorando mayusculas/minusculas), lo devolvemos tal cual
        if (path.EndsWith(dottedExtension, StringComparison.OrdinalIgnoreCase))
        {
            return path;
        }

        // Si no la tiene, se lo pegamos al final
        return $"{path}{dottedExtension}";
    }
}
